/**
 * TmuxLegends API server — optional player accounts (Google/GitHub OAuth) and
...
const SESSION_COOKIE = 'tmux_session'
const STATE_COOKIE = 'tmux_oauth_state'
...
export function loadConfig(env = process.env) {
  ...
  if (!sessionSecret) {
    ...
    console.warn(
      '[tmuxlegends-server] SESSION_SECRET is not set — using a random one-off secret. ' +
  ...
  return {
    ...
    dbPath: env.DB_PATH || './tmuxlegends.db',
    publicOrigin,
...
export function createApp(config) {
  ...
  async function authCallback(req, res, provider, url) {
    ...
    try {
      ...
      console.error(`[tmuxlegends-server] ${provider} oauth callback failed:`, err?.message ?? err)
  ...
  const server = createServer((req, res) => {
    handle(req, res).catch((err) => {
      console.error('[tmuxlegends-server] unhandled error:', err)
...
if (isMain) {
  ...
  app.server.listen(config.port, () => {
    console.log(
      `[tmuxlegends-server] listening on :${config.port}` +
